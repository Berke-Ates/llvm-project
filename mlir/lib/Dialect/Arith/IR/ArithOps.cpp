...
Operation *arith::ConstantOp::generate(GeneratorOpBuilder &builder) {
  llvm::DenseMap<Type, TypedAttr> attrMap = {
      {builder.getIndexType(),
       builder.getIndexAttr(builder.sampleNormal<int64_t>())},
      {builder.getI1Type(), builder.getBoolAttr(builder.sampleBool())},
      {builder.getI8Type(),
       builder.getI8IntegerAttr(builder.sampleNormal<int8_t>())},
      {builder.getI16Type(),
       builder.getI16IntegerAttr(builder.sampleNormal<int16_t>())},
      {builder.getI32Type(),
       builder.getI32IntegerAttr(builder.sampleNormal<int32_t>())},
      {builder.getI64Type(),
       builder.getI64IntegerAttr(builder.sampleNormal<int64_t>())},
      {builder.getF16Type(),
       builder.getF16FloatAttr(builder.sampleNormal<float_t>())},
      {builder.getF32Type(),
       builder.getF32FloatAttr(builder.sampleNormal<float_t>())},
      {builder.getF64Type(),
       builder.getF64FloatAttr(builder.sampleNormal<double_t>())},
  };
...
Operation *arith::ExtUIOp::generate(GeneratorOpBuilder &builder) {
  ...
  llvm::SmallVector<Type> possibleTypes(allTypes.begin(), allTypes.end() - 1);

  llvm::DenseMap<Type, llvm::SmallVector<Type>> typesMap;
  for (unsigned i = 0; i < possibleTypes.size(); ++i)
    typesMap[possibleTypes[i]] =
...
Operation *arith::CmpIOp::generate(GeneratorOpBuilder &builder) {
  llvm::SmallVector<CmpIPredicate> predicateTypes = {
      ...
      CmpIPredicate::uge, CmpIPredicate::ugt, CmpIPredicate::ule,
      CmpIPredicate::ult,
  };
...
Operation *arith::CmpFOp::generate(GeneratorOpBuilder &builder) {
  llvm::SmallVector<CmpFPredicate> predicateTypes = {
      ...
      CmpFPredicate::OLT,         CmpFPredicate::ONE,        CmpFPredicate::ORD,
      CmpFPredicate::UEQ,         CmpFPredicate::UGE,        CmpFPredicate::UGT,
      CmpFPredicate::ULE,         CmpFPredicate::ULT,        CmpFPredicate::UNE,
      CmpFPredicate::UNO,
  };
